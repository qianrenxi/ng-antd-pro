...
@Component({
  selector: 'demo-col-region',
  templateUrl: './col-region.component.html',
  styleUrls: ['./col-region.component.scss']
})
export class ColRegionComponent implements OnInit {

  @Input() widgets = []; // : ReportWidget[][];
  ...
  colsReceive($event) {
    console.log('cols receive', $event);
  ...
  colRemoveItem($event) {
    ...
    const list = container.sortData as any[];
      ...
      console.log('remove...')
      // TODO: 在同一个region组里边，可能会有先后顺序的问题，如果在remove之后再插入可能会导致位置错误
      // TODO： 目前没有发现此问题
      _.remove(this.widgets, (it) => _.isEmpty(it));
  }
